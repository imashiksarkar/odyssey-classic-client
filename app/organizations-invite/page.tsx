...
export default function OrganizationInvitePage() {
  ...
  useEffect(() => {
    const loadInvite = async () => {
      ...
      try {
        const nextInvite = await organizationApi.getPendingOrganizationInviteByInviteId(
          inviteId,
        );
  ...
  const handleInvitationAction = async (mode: "accept" | "reject") => {
    ...
    try {
      if (mode === "accept") {
        const nextInvite = await organizationApi.acceptOrganizationInvite(inviteId, accessToken);
        if (nextInvite) {
          setInvite(nextInvite);
        }
        setSuccessMessage("Invitation accepted. Redirecting to organizations...");
      } else {
        const nextInvite = await organizationApi.rejectOrganizationInvite(inviteId, accessToken);
        if (nextInvite) {
          setInvite(nextInvite);
        }
        setSuccessMessage("Invitation rejected. Redirecting to organizations...");
      }
    ...
    <div className="min-h-screen bg-[radial-gradient(circle_at_top,_rgba(33,96,178,0.18),_transparent_30%),linear-gradient(180deg,#05070d_0%,#090d16_100%)] px-4 py-6 text-white">
      <div className="mx-auto flex min-h-[calc(100vh-3rem)] max-w-6xl flex-col">
        <header className="flex items-center justify-between gap-4">
          <Link
            href="/"
            className="flex items-center gap-2 rounded-full border border-white/10 bg-white/5 px-4 py-2 text-sm text-white/80 transition hover:bg-white/8 hover:text-white"
          >
        ...
        <main className="flex flex-1 items-center justify-center py-10">
          <div className="w-full max-w-[560px] rounded-[32px] border border-white/10 bg-[#0d1119]/90 p-8 shadow-[0_28px_100px_rgba(0,0,0,0.5)] backdrop-blur">
            <div className="mx-auto flex h-20 w-20 items-center justify-center rounded-[28px] bg-[linear-gradient(180deg,#1f7ed0_0%,#0d4b84_100%)] text-3xl font-semibold">
              O
            </div>

            <div className="mt-8 text-center">
              <p className="text-sm uppercase tracking-[0.3em] text-white/40">Organization Invite</p>
              <h1 className="mt-4 text-3xl font-semibold tracking-tight">
                {invite?.organization?.name || "Respond to invitation"}
              </h1>
              <p className="mt-3 text-sm leading-7 text-white/55">
                Review the invitation linked to your account and choose whether to join or decline.
              </p>
            </div>
            ...
            <div className="mt-8 rounded-[22px] border border-white/8 bg-white/[0.03] p-5">
              ...
              <p className="mt-3 truncate rounded-full bg-white/8 px-4 py-3 text-center text-lg font-semibold">
              ...
              <p className="mt-4 break-all text-center text-xs text-white/35">
                Invite ID: {inviteId || "Missing"}
              </p>
            </div>
            ...
            {isPageLoading ? (
              ...
              <div className="mt-8 flex items-center justify-center gap-2 text-white/65">
                <X className="h-4 w-4" />
                <span>Missing invitation id.</span>
              </div>
            ) : !invite ? (
              <div className="mt-8 flex items-center justify-center gap-2 text-white/65">
                <X className="h-4 w-4" />
              ...
              <div className="mt-8 grid gap-3 sm:grid-cols-2">
                <Button
                  ...
                  disabled={actionMode !== ""}
                  className={cn(
                    "h-14 rounded-full text-base font-semibold",
                    "bg-white text-[#090d16] hover:bg-white/90",
                  )}
                >
                  {actionMode === "accept" ? "Accepting..." : "Accept invitation"}
                </Button>
                <Button
                  onClick={() => handleInvitationAction("reject")}
                  disabled={actionMode !== ""}
                  className={cn(
                    "h-14 rounded-full border border-white/14 bg-white/6 text-base font-semibold text-white hover:bg-white/10",
                  )}
                >
                  {actionMode === "reject" ? "Rejecting..." : "Reject invitation"}
                </Button>
              </div>
            ) : (
              <div className="mt-8 space-y-3">
                ...
                <p className="text-center text-sm text-white/50">
                  You need an active Odyssey session before you can accept or reject this invite.
                </p>
              </div>
            )}
          </div>
